...
export default function AddNewAddress(props) {
  ...
  const mapRef = useRef()
  const lastGeocodedRegion = useRef()
  ...
  const onRegionChangeComplete = useCallback(async (coordinates) => {
    ...
    if (lastGeocodedRegion.current === regionKey) return
    lastGeocodedRegion.current = regionKey
    ...
    try {
      ...
      // Keep the map usable when the geocoding service has no result.
    }
  }, [getAddress])
